...
class IndexTests: XCTestCase {
    ...
    func testParse() {
        ...
        let index = testRepository.index!
    ...
    func testIndexTreeDelta() {
        ...
        let index = testRepository.index!
        let treePath = testRepository.subpath(with: "objects/1f/9bcfa09c52c0e5c7df0aa6953ffff8dffdf3c5")
        let tree = try! Object.from(file: treePath, in: testRepository) as! Tree
    ...
    func testIndexWorkingDirectoryDelta() {
        ...
        let hiFile = testRepository.path + "hi.txt"
        try! "hi".writeToPath(hiFile)
        try! (testRepository.path + "third.txt").deleteFile()
        try! "overwritten".writeToPath(testRepository.path + "file.txt")
        
        let index = testRepository.index!
        let delta = IndexDelta(index: index, repository: testRepository)
...
func executeGitCommand(with additionalArguments: [String]) {
    let arguments = ["-C", testRepository.path.rawValue] + additionalArguments
